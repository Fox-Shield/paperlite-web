    ...
    <div class="builder-content">
      ...
      <div class="save-bar">
        <div class="save-bar-left">
          <h2 class="save-bar-title">{{ isEditing ? 'Edit Template' : 'New Template' }}</h2>
        </div>
        <div class="save-bar-right">
          <router-link to="/templates/my" class="cancel-link">Cancel</router-link>
          <button class="btn-save" :disabled="saving" @click="save">
            <svg v-if="saving" class="spin-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="split-panel">
        ...
        <div class="left-panel">
          <div class="panel-header">
            ...
            <button class="btn-add-field" @click="showFieldTypePicker = !showFieldTypePicker">
          ...
          <div class="field-list">
            ...
            >
              ...
              <div class="field-row-actions">
                <button class="icon-btn" title="Move up" @click.stop="moveFieldUp(index)" :disabled="index === 0">▲</button>
                <button class="icon-btn" title="Move down" @click.stop="moveFieldDown(index)" :disabled="index === fields.length - 1">▼</button>
                <button class="icon-btn danger" title="Delete" @click.stop="removeField(index)">✕</button>
              </div>
            </div>
          </div>
          ...
          <div v-if="selectedField" class="field-editor">
            ...
            <div class="form-group">
              <label>Label</label>
              <input v-model="selectedField.label" type="text" placeholder="Field label" />
            </div>

            <div class="form-group">
              <label>Placeholder</label>
              <input v-model="selectedField.placeholder" type="text" placeholder="Placeholder text" />
            </div>

            <div v-if="selectedField.type === 'SELECT'" class="form-group">
              <label>Options <span class="hint">(comma-separated)</span></label>
              <input v-model="selectedField.options" type="text" placeholder="Option A, Option B, Option C" />
            </div>

            <div v-if="selectedField.type !== 'CHECKBOX' && selectedField.type !== 'SIGNATURE'" class="form-group">
              <label>Default Value</label>
              <input v-model="selectedField.defaultValue" type="text" placeholder="Default value" />
            </div>
        ...
        <div class="right-panel">
          <div class="preview-settings">
            ...
            <select v-model="settings.fontSize" class="settings-select">
              <option v-for="size in FONT_SIZES" :key="size" :value="size">{{ size }}pt</option>
            </select>
            <select v-model="settings.pageLayout" class="settings-select">
              <option v-for="layout in PAGE_LAYOUTS" :key="layout" :value="layout">{{ layout }}</option>
            </select>
          </div>

          <div class="preview-area">
            <div class="preview-page" :style="{ fontFamily: settings.fontFamily, fontSize: settings.fontSize + 'pt' }">
              <!-- Template name -->
              ...
              <div class="preview-desc-row">
                <input
                  ...
                  :style="{ fontFamily: settings.fontFamily }"
                />
                <select v-model="templateCategory" class="template-category-select">
              ...
              <div v-if="fields.length === 0" class="preview-empty">
                <p>Add fields from the left panel to see a live preview</p>
              </div>
              ...
              <div v-else class="preview-fields">
                ...
                >
                  ...
                  <textarea
                    ...
                    readonly
                  />
                  <select v-else-if="field.type === 'SELECT'" class="preview-input preview-select">
                  ...
                  <div v-else-if="field.type === 'CHECKBOX'" class="preview-checkbox-row">
                    <input type="checkbox" :disabled="true" />
                    <span>{{ field.placeholder || field.label }}</span>
                  </div>
                  <div v-else-if="field.type === 'SIGNATURE'" class="preview-signature-box">
                    <span>✍ Signature</span>
                  </div>
                  <input
                    v-else
