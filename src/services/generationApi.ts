...
const generationApi = {
    async getFieldValues(documentId: number): Promise<FieldValue[]> {
        const { data } = await api.get<FieldValue[]>(
            `/documents/${documentId}/field-values`,
        )
        return data
    },
    ...
    async generateDocument(documentId: number): Promise<GenerationRequest> {
        const { data } = await api.post<GenerationRequest>(
    ...
    async getGenerationStatus(
        ...
        const { data } = await api.get<GenerationRequest>(
            `/documents/${documentId}/generation/${requestId}/status`,
        )
        return data
    },
    ...
    async downloadPdf(documentId: number, requestId: number): Promise<Blob> {
        const { data } = await api.get<Blob>(
            `/documents/${documentId}/generation/${requestId}/download`,
            { responseType: 'blob' },
        )
        return data
    },
